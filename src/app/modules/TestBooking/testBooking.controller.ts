...
const createTestBookingController = asyncHandler(async (req, res) => {
  ...
  const createdTestBooking =
    await TestBookingServices.createTestBooking(testBookingPayload);
...
const updateTestBookingStatusByIdController = asyncHandler(async (req, res) => {
  const id = req.params.id;
  const { status } = req.body;

  const updatedTestBookingStatus =
    await TestBookingServices.updateTestBookingStatusById(id, status);
