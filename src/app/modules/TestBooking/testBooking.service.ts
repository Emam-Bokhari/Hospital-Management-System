...
const createTestBooking = async (payload: TTestBooking) => {
    ...
    if (!test?.testAvailability) {
        throw new HttpError(400, "The requested test is currently unavailable. Please choose a different test or try again later.")
    }
...
const updateTestBookingStatusById = async (id: string, status: string) => {
    ...
    const updatedTestBookingStatus = await TestBooking.findOneAndUpdate({ _id: id }, { status }, { new: true, runValidators: true })
