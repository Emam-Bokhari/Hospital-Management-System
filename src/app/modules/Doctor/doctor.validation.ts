...
const createDoctorValidationSchema = z.object({
    body: z.object({
        ...
        previousWorkPlace: z.array(createPreviousWorkPlaceValidationSchema).optional(),
...
const updateDoctorValidationSchema = z.object({
    body: z.object({
        ...
        educationDetails: z.array(updateEducationalDetailsValidationSchema).optional(),
        qualifications: z
            .array(z.string().min(2).max(100), { required_error: "At least one qualification is required" }).optional(),
        licenseNumber: z.string().min(5, "License number must have at least 5 characters").optional(),
        licenseExpiryDate: z.string().regex(/^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$/, "Invalid date format (YYYY-MM-DD)")
            .refine((value) => new Date(value) > new Date(), "License expiry date must be in the future").optional(),
        previousWorkPlace: z.array(updatePreviousWorkPlaceValidationSchema).optional(),
        yearsOfExperience: z.number().max(50).optional(),
        ...
        offDays: z
            .array(z.enum(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'])).optional(),
        isDeleted: z.boolean().default(false),
    })
})
