...
const addressValidationSchema = z.object({
    division: z.string().min(2, "Division name is required and must be contain at least 2 characters").max(50, 'Division name cannot exceed 50 characters'),
    district: z.string().min(2, "District name is required and must be contain at least 2 characters").max(50, 'District name cannot exceed 50 characters'),
    subDistrict: z.string().min(2, "Sub district name is required and must be contain at least 2 characters").max(50, 'Sub district name cannot exceed 50 characters'),
})
...
export const createAppointmentBookingValidationSchema = z.object({
    body: z.object({
        ...
        lastName: z
            ...
            .optional(),
        age: z.number().int().min(1, "Age is required and must be contain at least 1").max(120, "Age cannot exceed 120"),
        weight: z.number().int("Weight must be an integer").min(3, "Weight must be at least 5 kg").max(300, "Weight cannot exceed 300 kg").optional(),
