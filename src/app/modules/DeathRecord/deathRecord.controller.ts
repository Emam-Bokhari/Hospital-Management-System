...
const createDeathRecordController = asyncHandler(async (req, res) => {
  ...
  const createdDeathRecord =
    await DeathRecordServices.createDeathRecord(deathRecordPayload);
...
const updateDeathRecordController = asyncHandler(async (req, res) => {
  ...
  const updatedDeathRecord = await DeathRecordServices.updateDeathRecordById(
    id,
    updatedPayload,
  );
...
const deleteDeathRecordController = asyncHandler(async (req, res) => {
  const id = req.params.id;
  await DeathRecordServices.deleteDeathRecordById(id);
  sendResponse(res, {
    success: true,
