...
const getAllBirthRecords = async () => {
  const birthRecords = await BirthRecord.find().populate({
    path: 'createdBy',
...
const getBirthRecordById = async (id: string) => {
  const birthRecord = await BirthRecord.findById(id).populate({
    path: 'createdBy',
    select: 'firstName lastName email role',
  });
...
const updateBirthRecordById = async (
  id: string,
  payload: Partial<TBirthRecord>,
) => {
...
const deleteBirthRecordById = async (id: string) => {
  const deletedBirthRecord = await BirthRecord.findOneAndUpdate(
    { _id: id, isDeleted: false },
    { isDeleted: true },
    { new: true },
  );
