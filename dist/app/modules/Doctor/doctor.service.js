...
const updateDoctorById = (id, payload) => __awaiter(void 0, void 0, void 0, function* () {
    const { contactInformation, emergencyContact, medicalPracticeInformation, educationDetails, awards, previousWorkPlace, workingHours, qualifications, offDays, workingDays } = payload, remainingDoctorData = __rest(payload, ["contactInformation", "emergencyContact", "medicalPracticeInformation", "educationDetails", "awards", "previousWorkPlace", "workingHours", "qualifications", "offDays", "workingDays"]);
    ...
    if (medicalPracticeInformation) {
        (0, flattenAndUpdate_1.flattenAndUpdate)('medicalPracticeInformation', medicalPracticeInformation, modifiedUpdatedData);
    }
    // Utility function to flatten nested fields, update array of object fields
    if (educationDetails && educationDetails.length > 0) {
        (0, updateArrayField_1.updateArrayField)('educationDetails', educationDetails, modifiedUpdatedData);
    }
    if (awards && awards.length > 0) {
        (0, updateArrayField_1.updateArrayField)('awards', awards, modifiedUpdatedData);
    }
    if (previousWorkPlace && previousWorkPlace.length > 0) {
        (0, updateArrayField_1.updateArrayField)('previousWorkPlace', previousWorkPlace, modifiedUpdatedData);
    }
    if (workingHours && workingHours.length > 0) {
        (0, updateArrayField_1.updateArrayField)('workingHours', workingHours, modifiedUpdatedData);
    }
    //  update array fields
    if (qualifications) {
        const currentQualifications = (doctor === null || doctor === void 0 ? void 0 : doctor.qualifications) || [];
        if (qualifications.length > 0) {
            modifiedUpdatedData.qualifications = Array.from(new Set([...currentQualifications, ...qualifications]));
        }
    }
    if (workingDays) {
        const currentWorkingDays = (doctor === null || doctor === void 0 ? void 0 : doctor.workingDays) || [];
        if (workingDays.length > 0) {
            modifiedUpdatedData.workingDays = Array.from(new Set([...currentWorkingDays, ...workingDays]));
        }
    }
    if (offDays) {
        const currentOffDays = (doctor === null || doctor === void 0 ? void 0 : doctor.offDays) || [];
        if (offDays.length > 0) {
            modifiedUpdatedData.offDays = Array.from(new Set([...currentOffDays, ...offDays]));
        }
    }
    const updatedDoctor = yield doctor_model_1.Doctor.findOneAndUpdate({ _id: id, isDeleted: false }, modifiedUpdatedData, { new: true, runValidators: true });
