...
const updateDepartmentById = (id, payload) => __awaiter(void 0, void 0, void 0, function* () {
    const department = yield department_model_1.Department.findById(id);
    if (!department) {
        throw new HttpError_1.HttpError(404, `No department found with ID: ${id}`);
    }
    const { symptomsAddressed, possibleCauses } = payload, remainingDepartmentData = __rest(payload, ["symptomsAddressed", "possibleCauses"]);
    ...
    if (symptomsAddressed && symptomsAddressed.length > 0) {
        (0, updateArrayField_1.updateArrayField)('symptomsAddressed', symptomsAddressed, modifiedUpdatedData);
    }
    if (possibleCauses && possibleCauses.length > 0) {
        (0, updateArrayField_1.updateArrayField)('possibleCauses', possibleCauses, modifiedUpdatedData);
    }
    const updateDepartment = yield department_model_1.Department.findOneAndUpdate({ _id: id, isDeleted: false }, modifiedUpdatedData, { new: true, runValidators: true });
    return updateDepartment;
});
const deleteDepartmentById = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const deletedDepartment = yield department_model_1.Department.findOneAndUpdate({ _id: id, isDeleted: false }, { isDeleted: true }, { new: true });
