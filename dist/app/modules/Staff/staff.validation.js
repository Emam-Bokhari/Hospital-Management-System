...
const updateEmergencyContactValidationSchema = zod_1.z.object({
    name: zod_1.z
        ...
        .min(2, 'Name is required and must contain at least 2 characters')
        .max(50, 'Name cannot exceed 50 characters')
        .regex(/^[a-zA-Z\-'\s]+$/, 'Name can only contain letters, spaces, hyphens, or apostrophes').optional(),
    phone: zod_1.z
        .string()
        .trim()
        .regex(/^\+\d{1,4}\d{7,15}$/, { message: 'Invalid phone number format!' }).optional(),
...
const createNidValidationSchema = zod_1.z.object({
    number: zod_1.z.string().trim().regex(/^\d{10}$/, {
        message: 'NID number must be exactly 10 digits.',
    }),
    scannedCopy: zod_1.z.string()
...
const createGuardianValidationSchema = zod_1.z.object({
    ...
    email: zod_1.z
        ...
        .max(100, 'Email cannot exceed 100 characters')
        .optional(),
    nidNumber: zod_1.z.string().trim().regex(/^\d{10}$/, {
        message: 'NID number must be exactly 10 digits.',
    }),
    nidScannedCopy: zod_1.z.string()
...
const updateWorkScheduleValidationSchema = zod_1.z.object({
    ...
    startTime: zod_1.z
        .string()
        .regex(/^(0[0-9]|1[0-9]|2[0-3]):([0-5][0-9])$/, 'Time slot must be in HH:mm 24-hour format').optional(),
    endTime: zod_1.z
        .string()
        .regex(/^(0[0-9]|1[0-9]|2[0-3]):([0-5][0-9])$/, 'Time slot must be in HH:mm 24-hour format').optional(),
});
const createPayrollInformationValidationSchema = zod_1.z.object({
    monthlySalary: zod_1.z.number({ invalid_type_error: 'Monthly salary must be a numeric value.' })
...
const createExperienceValidationSchema = zod_1.z.object({
    previousCompany: zod_1.z.string().trim().min(2, 'Previous company is required and must contain at least 2 characters')
        .max(100, 'Previous company cannot exceed 100 characters'),
    previousJobTitle: zod_1.z.string().trim().min(2, 'Previous job title is required and must contain at least 2 characters')
        .max(100, 'Previous job title cannot exceed 100 characters'),
    totalYearsOfExperience: zod_1.z.number({ invalid_type_error: 'Total years of experience must be a numeric value.' })
    ...
    reasonForLeaving: zod_1.z.string().trim().min(5, 'Reason for leaving is required and must contain at least 2 characters')
...
const updateExperienceValidationSchema = zod_1.z.object({
    ...
    reasonForLeaving: zod_1.z.string().trim().min(5, 'Reason for leaving is required and must contain at least 5 characters')
...
const createStaffValidationSchema = zod_1.z.object({
    body: zod_1.z.object({
        ...
        bloodGroup: zod_1.z
            ...
            .optional(),
        profilePicture: zod_1.z.string().max(500, 'Profile picture url can not exceed 500 characters')
            .optional(),
        ...
        dateOfJoining: zod_1.z.string().length(10)
...
const updateStaffValidationSchema = zod_1.z.object({
    body: zod_1.z.object({
        firstName: zod_1.z
            ...
            .min(2, 'First name is required and must contain at least 2 characters')
            .max(50, 'First name cannot exceed 50 characters')
            .regex(/^[a-zA-Z\-'\s]+$/, 'First name can only contain letters, spaces, hyphens, or apostrophes').optional(),
        lastName: zod_1.z
            .string()
            .trim()
            .min(2, 'Last name is required and must contain at least 2 characters')
            .max(50, 'Last name cannot exceed 50 characters')
            .regex(/^[a-zA-Z\-'\s]+$/, 'Last name can only contain letters, spaces, hyphens, or apostrophes').optional(),
        gender: zod_1.z.enum(["male", "female"]).optional(),
        dateOfBirth: zod_1.z
            .string()
            .length(10)
            .regex(/^\d{4}-\d{2}-\d{2}$/, 'Date of birth must be in YYYY-MM-DD format').optional(),
        bloodGroup: zod_1.z
            .enum(['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'])
            .optional(),
        profilePicture: zod_1.z.string().max(500, 'Profile picture url can not exceed 500 characters')
        ...
        employmentType: zod_1.z.enum(["fullTime", "partTime", "contractual"]).optional(),
        employmentID: zod_1.z.string().optional(),
        dateOfJoining: zod_1.z.string().length(10)
            .regex(/^\d{4}-\d{2}-\d{2}$/, 'Date of joining must be in YYYY-MM-DD format').optional(),
