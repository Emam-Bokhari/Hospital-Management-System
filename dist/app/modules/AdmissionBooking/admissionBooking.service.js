...
const createAdmissionBooking = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    const bed = yield bed_model_1.Bed.findById(payload === null || payload === void 0 ? void 0 : payload.bed);
    // check if bed is exists
    if (!bed) {
        throw new HttpError_1.HttpError(404, 'The specified bed could not be found.');
    }
    // check if bed is available
    if (bed.availabilityStatus &&
    ...
    const admissionBookingId = yield (0, admissionBooking_utils_1.generateAdmissionBookingId)();
    payload.id = admissionBookingId;
    const createdAdmissionBooking = yield admissionBooking_model_1.AdmissionBooking.create(payload);
    return createdAdmissionBooking;
});
const getAllAdmissionBookings = () => __awaiter(void 0, void 0, void 0, function* () {
    const admissionBookings = yield admissionBooking_model_1.AdmissionBooking.find();
    if (admissionBookings.length === 0) {
        throw new HttpError_1.HttpError(404, 'No admission booking were found in the database');
    }
    return admissionBookings;
});
const getAdmissionBookingById = (id) => __awaiter(void 0, void 0, void 0, function* () {
    const admissionBooking = yield admissionBooking_model_1.AdmissionBooking.findById(id);
    if (!admissionBooking) {
        throw new HttpError_1.HttpError(404, `No admission booking found with ID: ${id}`);
    }
    return admissionBooking;
});
const updateAdmissionBookingStatusById = (id, status) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (status === 'discharged') {
        ...
        const admissionBooking = yield admissionBooking_model_1.AdmissionBooking.findById(id)
            ...
            .lean();
        if (!admissionBooking) {
            throw new HttpError_1.HttpError(404, `Admission booking with ID: ${id} not found`);
        }
        ...
        if (!bed || !bed.price) {
            throw new HttpError_1.HttpError(400, 'Price not available for the assigned bed');
        }
    ...
    const updatedAdmissionBookingStatus = yield admissionBooking_model_1.AdmissionBooking.findOneAndUpdate({ _id: id }, updatedFields, { new: true, runValidators: true });
