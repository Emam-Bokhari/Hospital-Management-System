...
const createAppointmentBooking = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const isWithinWorkingHours = (startTime, endTime, timeSlot) => {
        const startMinutes = (0, appointmentBooking_utils_1.convertTimeToMinutes)(startTime);
        const endMinutes = (0, appointmentBooking_utils_1.convertTimeToMinutes)(endTime);
        const slotMinutes = (0, appointmentBooking_utils_1.convertTimeToMinutes)(timeSlot);
    ...
    const validateDoctorAvailability = (doctor, appointmentDay, timeSlot) => {
        ...
        if (!doctor.workingDays.includes(appointmentDay)) {
            throw new HttpError_1.HttpError(400, `Doctor is not available on ${appointmentDay}. Available days are: ${doctor.workingDays}`);
        }
        // Ensure doctor has working hours
        if (!doctor.workingHours || doctor.workingHours.length === 0) {
            throw new HttpError_1.HttpError(400, 'Doctor does not have working hours');
        }
        ...
        if (!isAvailable) {
            throw new HttpError_1.HttpError(400, `Time slot ${timeSlot} is not within any of the doctor working hours`);
        }
    };
    // get the appointment day
    const appointmentDay = (0, appointmentBooking_utils_1.convertDateToDay)(payload.appointmentDate);
    ...
    const existingAppointment = yield appointmentBooking_model_1.AppointmentBooking.findOne({
        doctor: doctor._id,
        appointmentDate: payload.appointmentDate,
        timeSlot: payload.timeSlot,
    });
    if (existingAppointment) {
        throw new HttpError_1.HttpError(400, 'This time slot is already booked for the doctor');
    }
    // TODO: check if payment is exist
    // generate and add  appointment booking ID
    const appointmentBookingId = yield (0, appointmentBooking_utils_1.generateAppointmentBookingId)();
    payload.id = appointmentBookingId;
    const createdAppointmentBooking = yield appointmentBooking_model_1.AppointmentBooking.create(payload);
...
const updateAppointmentBookingStatusById = (id, status) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const appointmentBooking = yield appointmentBooking_model_1.AppointmentBooking.findById(id).select('_id doctor');
    if (!appointmentBooking) {
        throw new HttpError_1.HttpError(404, `Appointment booking with ID: ${id} not found`);
    }
    ...
    //     }
    // }
    const updatedAppointmentBookingStatus = yield appointmentBooking_model_1.AppointmentBooking.findByIdAndUpdate(id, { status }, { new: true });
